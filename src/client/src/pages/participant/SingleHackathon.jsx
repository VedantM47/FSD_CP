        ...
        if (userData) {
          ...
          try {
            ...
            teamDetails = allTeams.find(team => 
              team.members.some(m => String(m.userId?._id || m.userId) === String(userData?._id || userData))
            );

            const currentMember = teamDetails?.members?.find(m => String(m.userId?._id || m.userId) === String(userData?._id || userData));
            ...
            const isLeaderOfTeam = teamDetails && String(teamDetails.leader?._id || teamDetails.leader) === String(userData._id);
            ...
            setIsPendingRegistration(memberStatus === 'pending' || memberStatus === 'invited');
      ...
      <div className="sh-layout">
        <div className="sh-main">
          <div className="sh-tabs-container">
            {['About', 'Timeline', 'Prizes', 'Rules', 'Teams'].map((tab) => (
          ...
          <div id="Teams" className="sh-content-card">
            ...
            <div className="sh-teams-grid">
              {loadingOpenTeams ? (
                ...
                <div className="sh-empty-teams">
                  ...
                  <button className="btn-secondary" onClick={() => navigate(`/user/hackathon/${id}/register`)}>Create Your Own Team</button>
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
                  {openTeams.map(team => (
                    <div key={team._id} className="sh-team-card-alt" style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '20px', background: '#fff' }}>
                      <h4 style={{ margin: '0 0 8px 0', fontSize: '1.2rem' }}>{team.name}</h4>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>{team.members?.filter(m => m.status === 'accepted').length || 0} Members</span>
                        <button 
                          className="btn-view-team"
                          onClick={() => navigate(`/user/hackathon/${id}/team/${team._id}`)}
                          style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '6px 12px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: '700', cursor: 'pointer' }}
                        >
                          View Details
                        </button>
                      </div>
            ...
            <button 
              className="btn-link" 
              style={{ marginTop: '20px', color: '#2563eb', fontWeight: '700', background: 'none', border: 'none', cursor: 'pointer' }}
              ...
              <div className="dashboard-view">
                <div className="dashboard-header">
                  {isPendingRegistration ? (
                    <div className="registered-badge-pill" style={{ background: '#FEF3C7', color: '#D97706' }}><span className="dot" style={{ background: '#D97706' }}></span>{isInvitedPending ? 'INVITE PENDING ACCEPTANCE' : 'PENDING APPROVAL'}</div>
                  ) : (
