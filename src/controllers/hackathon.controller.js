...
/* Public or Protected (depending on what all in the project team thinks) */

/* Functionality: Get Discussion History
  Fetches all chat messages for a specific hackathon, including sender details.
  
  Input: req.params.hackathonId
  Returns: List of discussion objects sorted by time
*/
export const getHackathonDiscussion = async (req, res, next) => {
  try {
    const { hackathonId } = req.params;
    
    // Fetch all messages
    const messages = await Discussion.find({ hackathonId })
      .sort({ createdAt: 1 })
      .populate('senderId', 'fullName email')
      .populate('parentId', 'message senderId');
